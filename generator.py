...
class Generator(nn.Module):
    def __init__(self, noise_size, image_size):
        ...
        self.norm0 = nn.BatchNorm1d(self.image_size * self.image_size)
        ...
        # Block 2
        self.conv2 = nn.Conv2d(130, 66, 1, 1)
        self.norm2 = nn.BatchNorm2d(66)

        # Block 3
        self.conv3 = nn.Conv2d(66, 65, 1, 1)
        self.norm3 = nn.BatchNorm2d(65)

        # Block 4
        self.conv4 = nn.Conv2d(65, 65, 1, 1)
        self.norm4 = nn.BatchNorm2d(65)

        # Block 5
        self.conv5 = nn.Conv2d(65, 33, 1, 1)
        self.norm5 = nn.BatchNorm2d(33)

        # Output
        self.conv6 = nn.Conv2d(33, 2, 1, 1)

        # Last activation
        self.tanh = nn.Tanh()

    def forward(self, noise, y):
        noise = self.project(noise).view(-1, self.image_size, self.image_size, 1)
    ...
    def conv_block(self, y, x, block, last=False):
        x = torch.cat([y, x], 3)
        conv = getattr(self, 'conv' + block)
        x = conv(x)

        if not last:
            norm = getattr(self, 'norm' + block)
            x = norm(x)
            return self.relu(x)
        else:
            return self.tanh(x)
